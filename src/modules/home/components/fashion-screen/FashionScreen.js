...
const imageList = [
    {imageSrc:pinkJacketImg, style: {top: -50, left: 0, height: 200}, transformTrigger: 0 },
    {imageSrc:shoeImg, style: {top: 20, left: '50%', height: 120}, transformTrigger: 0.2 },
    {imageSrc:arrowImg, style: {top: 300, left: 150, height: 100}, transformTrigger: 0.3 },
    {imageSrc:orangeJacketImg, style: {top: 400, left: '30%', height: 300}, transformTrigger: 0.4 },
    {imageSrc:shoeImg, style: {top: 680, left: -50, height: 120}, transformTrigger: 0.5 },
    {imageSrc:arrowImg, style: {top: 680, right: 200, height: 120}, transformTrigger: 0.6 },
    {imageSrc:maskImg, style: {top: 880, left: '30%', height: 120}, transformTrigger: 0.7 },
]

export default function FashionScreen({scrollContainerHeight}){
    const scrollRatio = useWindowOnScrollRatio({scrollContainerHeight, offsetSelector: '.fashion-screen'});

    return <div className={'fashion-screen'}>
        <div className={'images-container'}>
            {imageList.map(({imageSrc, style, transformTrigger})=>{
                return <img src={imageSrc} style={
                    {...style, transform: scrollRatio >= transformTrigger ? 'translateY(0px)':'translateY(100vh)' }
                } />
            })}
        </div>
